...
Но с одной оговоркой: на Vostok-4 летала [та же версия ПО](https://github.com/ArtemKiyashko/RpiProbeLogger/releases/tag/v1.0.1), что и на Vostok-3b. Все изменения, описанные тут, сделаны по результатам анализа телеметрии Vostok-4 и эта версия еще никуда не летала! По этой же причине бранч не влит в мастер.
...
while (true)
{
    ...
    {
        //collect sensor data and write report entity
    }
    Thread.Sleep(1000);
}
```
...
Этот пункт был подробно рассмотрен в первой части статьи, с тех пор механика поиска места запуска не изменилась. [FPLN](https://fpln.ru/) и [Landing Predictor](http://predict.habhub.org/) вам в помощь.
...
Как видите - давление в последнем запуск зафиксировано намного меньшее, чем в предпоследнем. Да, зависимость не линейная, это видно даже без графика, но зависимость есть и она точно не в пользу показаний с GoPro. Вот, посмотрите на график:
...
- Максимальная высота, зафиксированная RPI (дальше мы потеряли сигнал GPS, но видно, что зонд еще не падает): 2:07:10
...
Из планов на будущее обозначу следущие:
